...
font = ImageFont.truetype("/usr/share/fonts/truetype/DroidSansMono.ttf", fontsize, encoding="unic")
...
def create_ascii_video_2():
    ...
    saved_file_name=""
    frame_rate=str (vc.get(cv2.CAP_PROP_FPS))
    ...
    while rval:
        ...
        print(aalib_screen_width, aalib_screen_height)

        # aalib_screen_width = int(float(tty_w))
        # aalib_screen_height = int(float(tty_h))
        ...
        y = 0
        with open(working_dir+"/"+str(c)+".txt",'w') as f:
    ...
    #subprocess.call(["ffmpeg", "-i", "%d.png", "-r",  frame_rate, "-y", "-c:v", "libx264", "-preset", "slow", "-crf", "18", "-c:a", "copy", "-pix_fmt", "yuv420p", working_dir+"/output.mkv" ])
